...
export function SkillsChooser({ defaultSkill = 'jira', scrollTargetId, allowFullscreen = false }: { defaultSkill?: string; scrollTargetId?: string; allowFullscreen?: boolean }) {
  ...
  // Play only the active clip, restarting it from the top; pause the rest.
  useEffect(() => {
    SKILL_VIDEOS.forEach((s) => {
      const v = refs.current[s.key];
      if (!v) return;
      if (s.key === active) { try { v.currentTime = 0; } catch { /* not ready yet */ } const pr = v.play(); if (pr && pr.catch) pr.catch(() => {}); }
      else v.pause();
    });
  }, [active]);
